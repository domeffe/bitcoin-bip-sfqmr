...
Leaving BTC with compromised cryptography is equivalent to leaving a server online with an open SMTP vulnerability: attackers will inevitably exploit it to drain shared resources. Securing those outputs is simply the act of closing the vulnerability so they cannot be abused by unauthorized parties.
